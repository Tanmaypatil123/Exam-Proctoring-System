...
def upload_students_details(request):
    ...
    if request.method == 'POST':
        ...
        if file.name.endswith('.csv'):
            ...
            return render(request, 'upload_students_data.html',{'message': 'Unsupported file format'})

        # Extract specific columns
        if all(col in df.columns for col in ['First Name', 'Last Name', 'Email', 'Passing Year']):
            ...
            return render(request, 'upload_students_data.html',{'message': 'File uploaded and data saved successfully'})

        else:
             return render(request, 'upload_students_data.html',{'message': 'Required columns not found in the file'})
